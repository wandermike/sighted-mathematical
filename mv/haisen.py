...
class HessianApp:
    def __init__(self, root):
        ...
        self.point_size = tk.IntVar(value=50)
        self.point_density = tk.IntVar(value=10) # Lower value means more points (e.g., 5 = every 5th grid line)
    ...
    def create_ui(self):
        ...
        resolution_scale = ttk.Scale(range_frame, from_=20, to=150, variable=self.resolution, 
    ...
    def reset_settings(self):
        ...
        self.alpha.set(0.8)
        self.point_size.set(50)
        self.point_density.set(10)
    ...
    def _plot_function(self):
        ...
        try:
            ...
            if self.show_hessian_points.get():
                ...
                step = max(1, resolution // point_density)
    ...
    def draw_function_and_hessian(self, expr, x_min, x_max, y_min, y_max, view_angle=None):
        ...
        try:
            ...
            if self.show_hessian_points.get():
                point_density_step = max(1, self.point_density.get()) # Ensure step is at least 1
                point_size = self.point_size.get()
                convex_count, concave_count, saddle_count = 0, 0, 0

                # Iterate using steps based on density
                for i in range(0, resolution, point_density_step):
                    for j in range(0, resolution, point_density_step):
                        try:
                            x_val, y_val = X[j, i], Y[j, i]
