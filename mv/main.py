...
# 导入自定义主题和组件
from themes.futuristic_theme import COLORS, FONTS, apply_theme
from components.buttons import FuturisticButton, IconButton
from components.cards import InteractiveCard
from effects.animations import fade_in, slide_in, ParticleSystem

# 导入各个功能模块
from gailvlunn import ProbabilityApp
from trig_plot_app import TrigPlotApp
...
def open_app_window(parent, app_class, title, geometry="1200x800"):
    ...
    def perform_fade_in():
        ...
        for i in range(steps + 1):
            if not window.winfo_exists():  # 检查窗口是否仍然存在
                return
            alpha = i / steps
            window.attributes('-alpha', alpha)
            window.update()
            window.after(20)  # 20毫秒间隔
...
class StandardCourseUI:
    ...
    def create_scrollable_content(self):
        ...
        self.canvas_window = self.canvas.create_window((0, 0), window=self.button_frame, anchor="nw")
    ...
    def create_card_buttons(self, button_data):
        ...
        for i, (text, command) in enumerate(button_data):
            ...
            card.grid_propagate(False)  # 固定卡片大小
    ...
    def add_hover_effect(self, card):
        """添加卡片的悬停效果"""
        def on_enter(e):
            card.config(bg=COLORS["bg_light"])
        
        def on_leave(e):
            card.config(bg=COLORS["bg_medium"])
    ...
    def on_window_resize(self, event=None):
        ...
        if event is None or event.widget == self.root:
            ...
            if hasattr(self, 'canvas') and hasattr(self, 'canvas_window'):
                self.canvas.itemconfig(self.canvas_window, width=self.content_container.winfo_width())
...
class MainApp:
    def __init__(self, root):
        ...
        # 添加标题和科技感的装饰元素
        title_frame = tk.Frame(self.navbar, bg=COLORS["bg_medium"])
        ...
        self.indicator = tk.Canvas(
            title_frame, width=20, height=20, 
            bg=COLORS["bg_medium"], highlightthickness=0
        )
        self.indicator.pack(side=tk.LEFT, padx=10)
        self.indicator_dot = self.indicator.create_oval(5, 5, 15, 15, fill=COLORS["accent_primary"])
        ...
        try:
            ...
            if os.path.exists(logo_path):
                ...
                self.top_logo_image = ImageTk.PhotoImage(img)
                logo_label = tk.Label(
                    self.navbar, 
                    image=self.top_logo_image, 
                    bg=COLORS["bg_medium"]
                )
                logo_label.pack(side=tk.RIGHT, padx=15)
            else:
                log(f"找不到logo.ico文件: {logo_path}")
        except Exception as e:
            log(f"添加顶部Logo时出错: {e}")
    ...
    def animate_indicator(self):
        ...
        def update_indicator():
            ...
            # 更新缩放
            delta = 0.05 if growing else -0.05
            scale += delta
            
            # 重绘指示器
            self.indicator.delete(self.indicator_dot)
            center_x, center_y = 10, 10
            radius = 5 * scale
            self.indicator_dot = self.indicator.create_oval(
                center_x - radius, 
                center_y - radius, 
                center_x + radius, 
                center_y + radius, 
            ...
            # 每50毫秒更新一次
            self.indicator.after(50, update_indicator)
    ...
    def create_feature_cards(self):
        ...
        base_height = base_width * 1.2  # 保持黄金比例
        ...
        for i, card_data in enumerate(cards_data):
            ...
            card_container = tk.Frame(
                self.cards_frame,
                bg=COLORS["bg_light"],
                highlightbackground="#E74C3C",  # 红色边框
                highlightthickness=2,           # 边框粗细
                cursor="hand2"                  # 手型光标
            )
